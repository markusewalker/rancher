...
func (t *TokenStore) Update(ctx extcore.Context, token *ext.Token, opts *metav1.UpdateOptions) (*ext.Token, error) {
	...
	return t.SystemTokenStore.update(hasPermission, token, opts)
...
func (t *SystemTokenStore) update(hasPermission bool, token *ext.Token, opts *metav1.UpdateOptions) (*ext.Token, error) {
	currentSecret, err := t.secretClient.Get(TokenNamespace, token.Name, metav1.GetOptions{})
	if err != nil {
	...
	if !hasPermission && (token.Spec.TTL > currentToken.Spec.TTL) {
